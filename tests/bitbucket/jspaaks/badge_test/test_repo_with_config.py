...
def mocked_repo(mocker):
    with mocker:
        return Repo("https://bitbucket.org/jspaaks/badge-test", config_file=".howfairis-custom-config.yml")
...
class TestRepoWithConfig(Contract):
    ...
    def test_raw_url_format_string(self, mocked_repo):
        assert mocked_repo.raw_url_format_string == "https://bitbucket.org/jspaaks/badge-test/raw/master/{0}"
