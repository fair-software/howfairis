...
class TestCli(Contract):
    ...
    def test_with_nonexistent_path(self, invoke_cli):
        result = invoke_cli(
            [
                "https://gitlab.com/fair-software/repo1",
                "--path",
                "this/path/does-not-exist",
            ]
        )
        expected = load_snippets_from_local_data(__file__)[
            "/cli-with-nonexistent-path.txt"
        ]
        assert (
            "Did not find a README[.md|.rst] file" in result.stdout
        ), "Did not raise expected warning"
        assert (
            "expect the compliance to suffer" in result.stdout
        ), "Did not raise expected warning"
        assert result.exit_code == 1
        assert result.stdout == expected
